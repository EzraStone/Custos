...
CANDIDATES: dict[str, str] = {
    "work_per_request_variance": (
        ...
        "workloads, where every shipped signal overlaps or inverts. Scoped by "
        "`looks_interactive` on the stress corpus it separates by +0.154: the "
        "support copilot reads 0.300 against 0.146 for the knowledge-base "
        "assistant and 0.117 for the ops assistant. The IDE assistant reads "
        "0.310 and is outside that scope, because the predicate excludes "
        "anything reaching an MCP server — the classifier already has evidence "
        "for those.\n\n"
        "Unscoped it is worse than useless. Across every workload it separates "
        "by -0.840.\n\n"
        "Not adopted, for two reasons that both have to be fixed by data rather "
        "than by thinking harder.\n\n"
        "It fires hardest on a negative. The embedding service reads 1.134 and "
        "the sales copilot 0.514, both above every agent, which is where the "
        "-0.840 comes from. `looks_interactive` is an availability predicate "
        "that fixes it and was not written for this — it exists to count the "
        "workloads a scan cannot resolve — but a signal that has to be fenced "
        "off from most of the corpus to stop being wrong is still a signal "
        "being fitted.\n\n"
        ...
        "Every figure above is printed by `make candidates` and held to this "
        "text by a test. They were measured once by hand and written here, "
        "which made them unfalsifiable — and one of them was already wrong."
    ),
}
